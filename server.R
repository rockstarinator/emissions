...
plotter <- function(dfplot) {
        ...
        h1$subtitle(text = "Bubble Size - Billions of Kilograms")
        ...
        h1$yAxis(title = list(text = "Kilograms per Capita"), tickPositions = seq(0, 50000, 10000), min = 0, max = 50000)
        h1$plotOptions(bubble = list(maxSize = "300", 
                                     minSize = "5",
                                     tooltip = list(headerFormat = "<b>{series.name}</b><br>",
                                                    pointFormat = "{point.y} kilograms per capita <br> {point.z} billion kilograms total")))
